...
export default function SifreSifirlaPage() {
  ...
  // When redirected from email, Supabase sets a recovery session
  useEffect(() => {
    const init = async () => {
      const { data } = await supabase.auth.getSession();
      // If no session, it's likely the link is invalid/expired
      setReady(true);
      if (!data.session) {
        setMessage("Bağlantı geçersiz veya süresi dolmuş olabilir. Lütfen tekrar deneyin.");
      }
    };
    init();
  }, []);
      ...
      <div className="relative z-10 flex min-h-dvh items-center justify-center px-6 py-12">
        <div className="w-full max-w-md glass-card rounded-3xl p-8 shadow-2xl fade-up">
          <div className="flex flex-col items-center gap-2 mb-6">
            <Image src="/images/paketservisci.png" alt="PaketServisçi Logo" width={180} height={60} priority className="drop-shadow-lg" />
            <h1 className="text-2xl font-extrabold text-white">Şifreyi Sıfırla</h1>
          </div>

          {!ready ? (
            <p className="text-center text-white/90">Yükleniyor...</p>
          ) : (
            <form className="space-y-4" onSubmit={handleUpdate}>
              <div>
                <label className="block text-sm font-medium text-white mb-1">Yeni Şifre</label>
              ...
              <button type="submit" className="primary-btn" disabled={loading}>{loading ? "Güncelleniyor..." : "Şifreyi Güncelle"}</button>
            </form>
          )}
